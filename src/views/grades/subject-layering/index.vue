			...
			<div class="ui-table-main">
				<el-table v-loading.body="listLoading" :data="list" stripe border style="width: 100%"  :default-sort = "{prop: 'chineseScoringRate', order: 'descending'}" :max-height="screenHeight">
					...
					<el-table-column label='得分率' header-align='center'>
						<el-table-column sortable prop="tmpSchoolScoreRate.averageValue" label="平均分">
              <template scope="scope">
                {{scope.row.tmpSchoolScoreRate.averageValue.toFixed(2)}}
              </template>
            </el-table-column>
						<el-table-column sortable prop="tmpSchoolScoreRate.scoreRate" label="得分率">
              <template scope="scope">
                  {{(scope.row.tmpSchoolScoreRate.scoreRate*100).toFixed(2)}}%
              </template>
						</el-table-column>
						<el-table-column sortable prop="tmpSchoolScoreRate.averageRate" label="超均率">
              <template scope="scope">
                  {{(scope.row.tmpSchoolScoreRate.averageRate*100).toFixed(2)}}%
              </template>
            </el-table-column>
                  ...
                  <div :style="{color: scope.row.tmpSchoolScoreRate.progressValue < 0 ? 'red' : '#333'}">
					...
					<el-table-column label='优良率' header-align='center'>
						...
						<el-table-column sortable prop="tmpSchoolExcellentRate.excellentRate" label="优良率">
              <template scope="scope">
                {{(scope.row.tmpSchoolExcellentRate.excellentRate*100).toFixed(2)}}%
              </template> 
            </el-table-column>
              ...
              <template scope="scope">
                {{(scope.row.tmpSchoolPassRate.passRate*100).toFixed(2)}}%
              </template>
						</el-table-column>
                  ...
                  <div :style="{color: scope.row.tmpSchoolPassRate.progressValue < 0 ? 'red' : '#333'}">
					...
					<el-table-column label='低分率' header-align='center'>
						...
						<el-table-column sortable prop="tmpSchoolLowGradeRate.lowGradeRate" label="低分率">
              <template scope="scope">
                {{(scope.row.tmpSchoolLowGradeRate.lowGradeRate*100).toFixed(2)}}%
              </template>      
            </el-table-column>
