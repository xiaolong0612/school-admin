...
function hasPermission(roles, permissionRoles) {
  if (roles.indexOf(store.user.auth_type) >= 0) return true;
  return roles.some(role => permissionRoles.indexOf(role) >= 0)
}
// register global progress.
const whiteList = ['/login', '/login-type', '/authredirect', '/reset', '/sendpwd'];// 不重定向白名单
router.beforeEach((to, from, next) => {
  NProgress.start();
  if (store.getters.token) {
    if (to.path === '/login' || to.path === '/login-type') {
      next({ path: '/' });
    } else {
